...
\section{Dérivée fonctionnelle, transformée de Fourier, opérateurs à noyaux}
...
\end{equation} 
Dans le cas où $U = L^2(\R^d)^N$ et $V = \R$ alors $D_fF \in U'$ (espace des forme linéaires continues de $U$) et on sait qu'il existe, par le théorème de Frechet-Riesz, une quantité unique que l'on note $\delta F[f]/\delta f$ telle que 
\begin{equation}
...
\begin{align}
  \forall \hv \in U  \quad D_{\varphiv} F .\hv & = \int_{\qv} \derd{F}{\varphiv(\qv)} \int_{\rv} h(\rv) e^{-i\qv.\rv} \\
  \forall \hv \in U  \quad D_{\varphiv} F .h & = \int_{\rv} \int_{\qv} \derd{F}{\varphiv(\qv)} e^{-i\qv.\rv } \hv(\rv)
\end{align}
}
...
On appelle $S$ un operateur a noyaux, une application de $L^2(\R^d, \R^N)$, telle que pour tout $(i,j) \in \bbrac{1,N}^2$, il existe une application $A_{i,j} \in L^2(\R^d\times\R^d, \R)$ telle que,  pour tout $f \in L^2(\R^d, \R^N)$, et $x \in \R^d$ 
 \begin{equation}
 ...
 On identifiera alors dans les notations $S$ et $A$ indépendemment. Par Cauchy-Schwartz cette définition a bien  un sens. De plus, on peut montrer que $S$ et est un endomorphisme de $L^2(\R^d, \R^N)$. \\
...
Nous avons vu que $S$ définie comme précédemment était un endomorphisme de $L^2(\R^d, \R^N)$. Ainsi il est possible d'en définir la transformée de Fourier. Nous avons donc,
\begin{equation}
...
{\footnotesize
Ceci ce démontre en développant le calcul de la transformée de Fourier. Soit $\qv \in \R^d$,
\begin{align}
...
La composition de ces deux opérateurs est définie par,
\begin{equation}
...
\end{equation}
Par transformée de Fourier, par un calcul analogue à celui fait pour un simple noyaux, il vient,
\begin{equation}
...
\subsubsection{Trace d'un opérateur à noyaux}
...
\subsubsection{Dérivation de l'inverse d'une matrice}
...
\end{equation}
Il s'ensuit alors directement le résultat.
}


\vspace*{11pt}
\subsubsection{Formules en vrac}
...
\subsection{Application aux tenseurs  $G^{(n)}$ et  $\Gamma^{(n)}$}
