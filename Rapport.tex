...
\partial_t \Gamma_{k}	[\phiv] = \frac{1}{2} \sum\limits_{i=1}^{N} \int_\qv \partial_t \Rc_k(\qv) \, G_{k, ii}[\qv; \phiv] \, ,
	\label{eq:flot}
\end{equation}
et nous avons aussi une condition initiale  en $k = \Lambda$ pour $\Gamma$ puisque, en définissant $\varphiv_0$ comme en \refeq{meanPhi}, d'après \refeq{MeanZ} il vient la relation
\begin{equation}
...
Le coefficient $S_{d-1}$ est une pré-facteur un réel. De ce fait, le calcul d'une intégrale à d dimensions se ramène au calcul de deux intégrales seulement, qui peut être fait sans problèmes avec une interpolation de Gauss-Legendre.
...
\end{equation}
Ainsi l'expression de $\lambda_\mu$ se trouve être 
\begin{equation}
...
\subsection{Application du NPRG et BMW}

\subsubsection{Régulateur}
...
\end{equation}
Avec $\varepsilon_k = \| \eo \|_\infty k^2$. Les expressions de $\partial_t \Rc_k$ se trouvent en \refann{}. Notons aussi que dans certaines équations que nous avons utilisées le facteur $Z_k$ a pu être omis car il ne sert qu'à avoir des dimensions compatibles lors de l'adimensionnement des équations. 
...
On peut réécrire \refeq{} adaptée avec la relation de dispersion ici selon
\begin{equation}
...
Pour des raisons pratiques nous avons décomposé la résolution numérique des équations BMW du modèle d'Ising en 3 étapes, en procédant, comme détaillé ci-après, à un adimensionnement progressif des équations. \\



\subsubsection{Première étape}

En effet, comme nous travaillons sur un réseau, les fonctions que l'on considère sont alors $2\pi$ périodiques selon les composantes des impulsions lorsque l'on considère les variables et les fonctions dimensionnées. (i.e. les fonctions et variables notées de manière générale sans tilde). Ceci permet de s'affranchir de certaines approximations numériques dans le calcul des intégrales. Il est donc préférable de commencer par résoudre le système d'équation BMW brut, noté $\Ec_1$, sans réaliser d'adimensionnement (équivalent à \refeq{}) pour $k $ variant de $\Lambda$ à $k_a$ (ou $k_a$ a une valeur déterminée). Le problème BMW à résoudre est donc \\

\noindent
{\itshape Trouver $\Delta_k$, $X_k$, solution de $\Ec_1$, i.e tels que pour tout $p_x \in [0, \pi]$, $p_y \in [0, \pi]$, $\phi \in [0, \infty[$, $k\in [k_a, \Lambda]$}
...
Où l'on note toujours $X_k(\phi) = \partial_{\phi}^2 V_k(\phi)$. Cette équation est très similaire à \refeq{} mais elle est écrite selon la variable $\phi$ et les fonctions dépendent non plus de la norme des impulsions mais bien des deux composantes de $\pv$, que sont $p_x$ et $p_y$ en dimension $d=2$. Les notations $J$ et $I$ correspondent à celle du modèle $\varphiv^4$ et sont écrites en \refann{}. \\


Les conditions initiales de ce problème viennent de la décomposition \refeq{}, ainsi que de \refeq{}.
\begin{equation}
...
\begin{align}
	...
	\end{aligned}
\end{align}
On pourra se référer à \refann{} pour les différentes notations. Les conditions initiales sont
\begin{equation*}
...
\noindent
{\itshape Trouver $\tY_k$, $\tX_k$, solution de $\Ec_3$, i.e tels que pour tout $\tp_x \in [0, \tp_{max}]$, $\tp_y \in [0, \tp_{max}]$, $\tphi \in [0, \infty[$, $k\in ]0, k_b]$}
...
\end{equation}
On pourra ici aussi se référer à \refann{} pour les différentes notations et plus de précision. Les conditions initiales sont
\begin{equation*}
\tY_{k_b}(\tp_x, \tp_y,\tphi) = \frac{1}{Z_k}\( 1+ \frac{\bDelta_{k_b}(\tp_x, \tp_y, \phi)}{k_b^2(\tp_x^2 +\tp_y^2)}\) -1
\end{equation*}
\begin{equation*}
...
En conclusion ici l'important est cette résolution numérique des équations que l'on découpe en trois parties distinctes pour utiliser au mieux les symétries du problème.
...
Afin de pouvoir calculer avec précision les intégrales $J$, $\bJ$, $\tJ$ apparaissant dans les équations nous avons choisi d'interpoler la fonction $\Delta_k$ ainsi que ces dérivées dans le plan des impulsions. Comme annoncé, pour cela nous réutilisons encore une interpolation de Tchébytchev adaptée en dimension 2. 
...
Ce théorème et sa démonstrations se trouvent dans l'ouvrage \cite{Tchebychev, Masson1980} dans le cas $a=-1$, $b=1$. La généralisation à $a$ et $b$ ($b>a$) quelconques est immédiate. Considérons le schéma semi-discrétisés en temps des équations.Pour des raisons similaires à ce qui a été fait en \refsec{} (\refann{}) nous travaillons alors avec des fonctions de classe $\Cc^\infty$ (donc $\Cc^1$) selon les variables $\phi$, $p_x$ et $p_y$ (ou leur équivalent adimensionné). Il vient que la condition de variations bornées est satisfaite \cite{Masson1980}.
...
Or, pour étudier simplement ces schéma nous les appliquons à une équation d'advection du type $\partial_x u + a \partial_t u = 0$ et nous en faisons une analyse de Von-Neumann. Il est donc possible de calculer les facteurs d'amplification de ces schémas comme en \refig{FacAmp}
