...
Nous rappelons ici de manière synthétiques les principales idées du RG développé par Wilson \cite{wilson}. Commençons pour cela par définir la transformée de Fourier du champ $\rv \rightarrow \varphiv(\rv)$ par 
\begin{equation}
\begin{split}
\varphiv(\pv) = \frac{1}{\sqrt{|\Omega|}}\int_\rv \varphiv(\rv) \, e^{-i \pv . \rv}, \\
\varphiv(\rv) = \frac{1}{\sqrt{|\Omega|}}\sum_\pv \varphiv(\pv) \, e^{i \pv  .\rv}
\end{split} 	
\end{equation}
Avec la notation 
\begin{equation}
...
Notons que c'est la variable utilisée $\rv$ ou $\pv$ (ou aussi parfois $\qv$ pour l'impulsion) qui nous permet de savoir si l'on travaille avec la fonction considérée ou sa transformée de Fourier . On suppose que pour $\pv$ supérieur à une certaine valeur notée $\Lambda$, la valeur $\varphiv(\pv)$ est suffisamment faible pour être pris comme nul, on considère donc $\pv \in [0, \Lambda]$. 

L'idée du RG est alors de ne pas considérer tous les degrés de liberté sur le même pied d'égalité. En effet, on commence d'abord, pour calculer $\Zc$, par intégrer les degrés de libertés de haute impulsion $\pv$ entre $k = \Lambda/s$ et $\Lambda$ où $k \in [0,\Lambda]$ et $s>1$. En pratique on sépare $\varphiv$ en deux fonctions $\varphiv_>$ et $\varphiv_<$ telles que $\varphiv(\pv) = \varphiv_>(\pv) + \varphiv_<(\pv)$ et
\begin{align}
...
\begin{equation}
	H_k[\varphiv_<] \equiv \int \Dc \varphiv_>  \exp \{ H[\varphiv_>+ \varphiv_<] \},
\end{equation}
et de réécrire la fonction de partition comme 
\begin{equation}
\Zc = \int \Dc \varphiv_< \, \exp\left\{- H_k[\varphiv_<]\right\}. 
\end{equation} 
De manière générale on part d'un hamiltonien qui ne dépend pas seulement du champ $\varphiv$ mais entendu aussi de constante de couplage qui caractérise la physique a laquelle est soumise le champ. Ces constantes sont notées $\{g_i\}_i$ avec $i \in \N$. On écrit alors $H[\varphiv] = H[\varphiv, \{g_i\}]$. Ainsi le nouvel hamiltonien $H_k$ possèdent de nouvelles constantes de couplages notées  $\{g_{k,i}\}_i$. 

On considère alors plutôt une intégration infinitésimale entre $[\Lambda - d\Lambda, \Lambda]$. On obtient alors un nouvel hamiltonien $H_{\Lambda - d\Lambda}$. Dans ce nouvel hamiltonien, sans changer sa valeur, on réalise un adimensionnement : toutes les longueurs sont multipliées par $k_{1} = \Lambda - d\Lambda$ et toutes les impulsions sont divisées par $k_{1}$. De plus on multiplie aussi par les bon facteur de $k_{1}$ pour que les constantes $\{g_{\Lambda - d\Lambda, i}\}$ se retrouvent sans dimensions. Ceci n'est que technique et ne change pas la valeur des expressions que l'on manipule mais elle permet cependant de faire comme si $H_{\Lambda - d\Lambda}$ était l'hamiltonien d'un nouveau système dans lequel toutes les échelles on été changée, et notamment les échelles de longueurs qui on été réduites. On rappelle qu'a la transition de phase du second ordre la longueur $\xi$ diverge, et donc notre système originel n'a plus vraiment d'échelle caractéristique de longueur. A la transition ce nouveau système est le premier possèdent donc bien la même physique.

 Pour passer réaliser l'intégration complète sur $[0, \Lambda]$ on peut donc itérer ce processus de transformations infinitésimales. On introduit à l'itération $p \in \N$ - itération à laquelle on a intégré sur $[k_p, \Lambda]$ - l'opérateur $T_p$ qui envoie $\{g_i\}_i$ sur $\{g_{k_p,i}\}_i$. L'hypothèse fondamentale du RG, qui n'est pas prouvée mais toujours vérifiée, est qu'il existe un point fixe de $T_p$ pour $p \to \infty$, si l'on est à la température critique (et/ou pression critique, ...). Ceci signifie plus particulièrement qu'il existe $\{g^*_i\}_i$ tel que 
 
 \begin{equation}
 	\lim\limits_{k \rightarrow \infty}  g_{k,i} = \lim\limits_{\substack{p \to \infty \\ d\Lambda \to 0 }}  g_{k_p,i} =  g^*_i
 \end{equation} 
 
 L'existence de ce points fixe explique alors l'universalité de exposants critiques \cite{Delamotte2012}. En effet, deux systèmes avec les mêmes propriétés de symétries auront le même point fixe et donc un comportement identique à la transition de phase, quand la longueur de corrélation est infinie. \\
 
 Dans un calcul de RG on prend classiquement un nombre fini de constantes de couplage et on considère qu'elles sont suffisamment faibles pour faire des développement du hamiltonien en puissance de ces constantes à chaque itération. Ceci permet de faire des calcul mais limite les applications possibles et n'est pas forcément obligatoire, ce pourquoi on introduit le NPRG. Nous ne nous étendrons donc pas plus sur les calculs dans le RG.

\vspace*{11pt}
\subsection{Le groupe de renormalisation non perturbatif (NPRG)}
\subsubsection{Généralités}

Contrairement au RG le NPRG développé par Wetterich \cite{wetterich} permet d'aller plus loin puisqu'il permet un calcul sans approximations a priori. L'idée reste la même que dans le RG en intégrant les hautes impulsions en  premier lieu. Cependant toute l'astuce consiste à faire cela en introduisant une fonction $\Rc_k \in \Cc^\infty(\R^{d},\R)$ appelée régulateur pour $k \in [0, \Lambda]$ puis une nouvelle fonction de partition modifiée  
\begin{equation}
...
On doit choisit alors la fonction $\Rc_k$ le plus astucieusement possible.
En effet, pour être conforme à la méthode du RG, il faut que l'on retrouve $\lim_{k \to 0}\Zc_k = \Zc$ et pour $k = \Lambda$ que l'expression de $\Zc_\Lambda$ puisse être calculée analytiquement. Il faut donc, 
\begin{itemize}
  \item Pour $k \rightarrow 0$, $\forall \qv$ $\, \Rc_{k, ij}(\qv) \rightarrow 0$  .
  \item Pour $k = \Lambda$, $\forall \qv$ $\, \Rc_{k, ij}(\qv)  \rightarrow  +\infty$ .
\end{itemize}

Et il faut que l'évolution entre ces deux configurations soit continue.
Ainsi, quand $k \to 0$, $\Delta S_k \to 0$ et l'on retrouve bien l'expression de $\Zc$. De plus quand $k = \Lambda$ prendre $\Rc_k$ comme étant infini permet de dire que ... 

Etant donné que numériquement nous ne pouvons pas prendre en compte un $\Rc_k$ qui va vers l'infini quand $k = \Lambda$ nous nous contenterons de prendre $\Rc_\Lambda$ comme une fonction minorée par une valeur bien supérieure aux valeurs de constantes de couplage de H . En pratique il suffit de prendre $\Rc_\Lambda \sim \Lambda^2$.

On pourrait prendre comme bon régulateur une fonction de Heaviside. Cependant, dans l'approximation BMW les régulateurs de Heaviside ne sont pas assez régulier et donnent des schéma numériques instables. On prend donc une expression définie sur $[0, \Lambda]$ de la forme
\begin{equation}
	\Rc_k(q) = \alpha \frac{q^2}{\exp{(q^2/k^2)}-1}
\end{equation}
Avec $\alpha$ un réel positif que l'on prend de l'ordre de l'unité et que l'on peut faire varier mais qui n'est pas supposer changer les résultats des calculs. Remarquons que l'on satisfait bien aux hypothèses imposées sur le régulateur car 
\begin{equation}
...
\begin{equation}
	V_k(\phi) = \frac{1}{|\Omega|} \Gamma_k[\phiv] 
\end{equation}


On peut alors utiliser toutes les définitions que l'on vient de poser pour extraire l'équation fondamentale du NPRG que l'on appelle l'équation de flot. Il s'agit d'une équation valable pour $k \in ]0, \Lambda]$ décrivant l'évolution de $\Gamma$ avec $k$ lorsqu'on le fait évoluer de $\Lambda$ à $0$. Cette équation permet donc d'approcher ...
\begin{equation}
\partial_t \Gamma_{k, ij}	[\phiv] = \frac{1}{2|\Omega|} \sum_\qv \partial_t \Rc_k(\qv) \, G^{(2)}_{k, ij}[\qv, -\qv, \phiv] \, ,
\end{equation}
...
En pratique, on suppose que comme dans la limite thermodynamique on étudie des systèmes macroscopiques et que $1/\Lambda$ est une grandeur microscopique $|\Omega|\gg 1/\Lambda^d$. Ainsi on fait l'approximation physique selon laquelle les impulsions ne sont plus de variables discrètes mais des variables continues et les sommes deviennent des intégrales avec la relation  
 
\begin{equation}
	\frac{1}{|\Omega|} \sum_{\qv} \rightarrow \frac{1}{(2\pi)^d} \int_{\R^d} ... \, \dd^d \, \qv  \equiv \int_\qv 
\end{equation}
