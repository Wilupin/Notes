...
Le but de cette étude est d'appliquer l'approximation BMW pour sur des systèmes connus grâces à d'autres méthodes afin de valider la qualité de cette approximation, de la méthode et de la résolution numérique.\\
...
\begin{theorem}
  Soient $a$, $b$ deux réels tels que $b>a$. Soit $f : [a,b]^2 \rightarrow \C$ une fonction continue aux variations bornées (comme définies en \cite{Tchebychev}). On suppose que l'une des dérivées partielles de de $f$ existe et est bornée dans $[a,b]$. Alors il existe une suite des coefficients réels $\{c_{ij}\}_{(i,j) \in \N^2}$ tels que la série $f_n$ définie par :
\begin{equation}
...
Et on adopte encore la même quadrature que pour les intégrales $I$, $\bI$, $\tI$. Cependant remarquons qu'il va nous falloir connaitre les valeurs de $u$ en des points $(x\pm b, y\pm c)$ qui sont en dehors du carré $[0, a]^2$ et à cause de la discrétisation de $u$, comme $u$ est une fonction que l'on connait par sa décomposition en série de tchebytchev sur $[0,a]^2$, on ne peut pas connaitre la valeur de $u$ en tout $(x\pm b, y\pm c)$. Lorsque l'on s'intéresse au calcul de $J$ cela ne pose pas de problèmes car on peut utiliser la périodicité des fonctions (raison pour laquelle nous résolvons en premier lieu le système $\Ec_1$ dimensionné). En revanche pour $\bJ$ et $\tJ$ , la périodicité est brisée dans les équations et nous sommes obligé d'extrapoler le comportement de $u$ en dehors de $[0,a]^2$
