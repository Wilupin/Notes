...
Ceci justifie alors l'utilisation des polynômes de Tchebytchev pour l'interpolation des fonctions dépendant d'une impulsion. En effet les fonctions semi-discrétisée en temps étant $\Cc^\infty$ sur $[0, \infty[$ elles le sont sur $[0, p_{max}]$. On utilise donc le théorème précédent pour $a=0$ et $b = p_{max}$. \\
...
\end{center}
\caption{Évolution du temps de calcul $\tau_c$ avec le nombre $n_{proc}$ de processeurs utilisés.}
\label{fig:timeScal}
\end{figure}
...
\begin{equation}
	...
	- \frac{1}{2}  I_2(\phi) \, \partial_\phi^{2} \Gamma_{k}^{(2)}(\pv, \phi)
\end{equation}
Soit $u$ une application affine, difféomorphisme de classe $\Cc^1$ de $\R^d$ dans $\R^d$ tel que $\text{det(Jac(}u)) = \pm 1$. Supposons que $\Rc_k(u(\qv)) = \Rc_k(\qv)$ pour tout $\qv \in \R^d$ et $k \in ]0, \Lambda]$. On pose le schéma semi-discrétisé d'Euler en temps de l'équation précédente. Il vient, pour $n \in \N$,
\begin{equation}
...
\begin{split}
	...
	\end{split}
\end{equation}
De plus ceci fonctionne aussi pour montrer la périodicité des fonctions et montrer que dans le cas Ising 2D elles peuvent bien être définies sur $\R^2$ tout entier,
\begin{equation}
\begin{split}
	u & : (p_x, p_y) \in \R^2 \to (p_x + 2\pi, p_y) \in \R^2 \\
	u & : (p_x, p_y) \in \R^2 \to (p_x, p_y+ 2\pi) \in \R^2 
	\end{split}
\end{equation}
