...
Bien entendu il n'est pas directement possible de calculer $H_k$ pour $k$ quelconque, sinon le problème serait résolu en calculant $H_0$. On considère alors plutôt une intégration infinitésimale entre $[\Lambda - d\Lambda, \Lambda]$ pour obtenir le nouvel hamiltonien $H_{k_1}$, où $k_1 = \Lambda - d\Lambda$. On introduit aussi $s_1 = \Lambda/k_1$. Ce calcul là, contrairement au calcul direct pour $k$ quelconque, peut être réalisé grâce à des approximations comme nous le détaillerons ensuite. Maintenant, avec l'expression de ce nouvel hamiltonien, on réalise ce que l'on appelle un adimensionnement qui se déroule en trois étapes listées ci-dessous.
 ...
 \end{equation}
La fonction de partition (dont il ne sert en fait à rien de connaitre le préfacteur numérique) est alores simplement exprimée comme 
\begin{equation}
...
\end{center}
\vspace*{-22pt}
\caption{(a) Allure du régulateur choisi pour une valeur de $k$ donnée avec une décroissance exponentielle (ligne continue bleue - c.f. \refeq{RegExp}) et avec une fonction de Heaviside (pointillés rouges -c.f. \refeq{RegHeav}). Ainsi ici nous avons approximativement intégrés les degrés de libertés correspondant à une impulsion dans $\simeq [k,\Lambda]$ uniquement. (b) Allure de la dérivée du régulateur choisi pour une valeur de $k$ donnée avec une décroissance exponentielle (ligne continue bleue, \refeq{regExp}). L'intégrale sur $\qv$ ne doit donc être faite seulement sur les $q$ appartenant dans la région rouge.}
\label{fig:RegDerReg}
\end{figure}
...
Nous écrivons ici ce que donne les équations dans le cas particulier $O(1)$, i.e. lorsque $H[-\varphiv] = H[\varphiv]$. Nous verrons un exemple plus précis en \refsec{}. Le cas général des équations pour $N$ quelconque se trouve dans \cite{benitez2012nonperturbative}.\\ 
...
\end{equation}
Dans cette équation il n'apparait que $\Gamma^{(2)}$, le problème de l'équation de flot exacte insoluble est résolu. 
...
On utilise à partir de maintenant la notation $\rho = \phi^2/2$. En outre, par la forme de l'hamiltonien les différentes grandeurs que la considère ne dépendent plus entièrement de l'impulsion $\pv$ (ou $\qv$, ou $\pv+\qv$) mais seulement de $\pv^2$ (ou $\qv^2$, ou $(\pv+\qv)^2$et donc de la norme $p = \| \pv \|_{2}$ (ou q, ou $\|\pv+\qv\|$). \\

Parce que l'on a \refeq{}, et parce qu'il est plus précis d'utiliser en complément de l'équation sur $\Gamma^{(2)}_k$ l'équation de flot de $V_k$, on introduit une fonction $\Delta_k(p, \rho)$ telle que
\begin{equation}
...
Pour la discrétisation en impulsion nous avons choisi d'utiliser des polynômes de Tchebytchev. En effet, nous n'avons aucune connaissance de la régularité des fonctions avec lesquelles nous travaillons cependant considérons simplement le schéma de résolution semi-discrétisé en espace. On note $\tY_{k_n}(\tp, \trho)$ la discrétisation en temps de la fonction $\tY$ au temps $t_n$. Alors pour tout $n \in \mathbb{N}$ la fonction $\tY_{k_n}$ est une fonction de classe $\Cc^{\infty}$ sur $[0, +\infty[^2$ (c.f. la démonstration \refann{}).

Or dans \cite{Tchebychev} il est démontré le théorème 
\begin{theorem}{Interpolation de Tchebychev}\\
...
\begin{equation}
	\forall x \in [a,b] \quad \text{Tch}_n[f](x) = \sum\limits_{j = 0}^{n-1} c_j T_j(x)
\end{equation}
Ainsi on montre que
\begin{equation}
\text{sup} |f(x) - \text{Tch}_n[f](x)| \le \frac{(b-a)^{n}}{2^{2n-1} n!}\text{sup} |f^{(n+1)}(x)|
\end{equation}
\end{theorem}

Ceci justifie alors partiellement l'utilisation des polynômes de Tchebytchev pour l'interpolation des fonctions dépendant d'une impulsion. En effet les fonctions semi-discrétisée en temps étant en $\Cc^\infty$ sur $[0, \infty[$ elles le sont sur $[0, p_{max}[$ où $p_{max}$ est l'impulsion maximale utilisée numériquement.
