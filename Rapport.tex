...
\end{equation}
Ceci sera utile pour obtenir une condition initiale de nos équations (c.f. \refeq{}). Et on obtient alors,
\begin{equation}
...
Notons aussi que l'intégrale dans cette équation de flot est en fait tronquée par la fonction $q \to \partial_t\Rc_k(q)$ qui pour un $k$ fixé limite, par sa décroissance, l'intervalle utile d'intégration qui n'est donc jamais véritablement faite sur un domaine infini et peut sans problème être réalisé numériquement comme ilustré en \refig{RegDerReg} dans le cas simple ou $\Rc_k(\qv) = \Rc_k(q)$.\\
...
Ainsi, pour diminuer les erreurs commises par l'approximation BMW et utiliser les informations exactes que l'on connait sur l'évolution de $V_k$ en prenant directement $\phiv = \phi$ uniforme dans \refeq{flot}, l'équation de flot BMW \refeq{flotBMW} peut se réécrire sous la forme du système couplé
\begin{align}
...
La condition initiale se déduit de \refeq{}. En effet, comme le cas ou l'on traite des champs $\phi$ uniforme alors le $\varphi0$ définissant le champ moyen est défini avec la contrainte
...
Pour chercher un point fixe on résout le système différentiel précédent pour différentes valeurs de $r_0'$. On teste donc plusieurs $r_0'$ avec une dichotomie avec des valeurs initiales prises aléatoirement jusqu'à ce que l'on arrive à trouver un point fixe.
...
\subsection{Décomposition sur les coefficients}
