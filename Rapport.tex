...
\end{equation}
où $t$ est le temps de RG, défini par $t = \ln(k/\Lambda)$. Avec $G^{(2)}_{k, ij}[\qv, -\qv, \phiv]$ la transformée de Fourier du tenseur $G_k^{(2)}$ définie en \refann{outils}. 
\begin{equation}
G^{(2)}_{k, ij}[\qv, -\qv, \phiv] = {\(\Gamma^{(2)}_{k, ij}[\qv, -\qv, \phiv]  + \Rc_k(\qv) \)}^{-1}
\end{equation}
En outre, nous avons aussi une condition initiale pour $\Gamma$ puisque d'après \refeq{MeanZ} il vient
\begin{equation}
...
Enfin, cette équation est pour l'instant dite dimensionné car lorsque l'on fait évoluer $k$ entre $k_1+dk_1$ et $k_1$ avec $a$
...
\begin{equation}
	J_n(p,\phi) = \int_q \partial_t \Rc_k \,G_k(p+q, \phi) \,G_k^{n-1}(q, \phi)
\end{equation}
\begin{equation}
	I_n(p,\phi) = \int_q \partial_t \Rc_k \,G_k^{n}(q, \phi)
\end{equation}
...
\end{equation}
Où $\varepsilon(\pv)$ est la relation de dispersion qui vaut simplement $\pv^2$ (venant de la TF du terme en gradient dans le hamiltonien). De par la forme de ce hamiltonien et de cette relation de dispersion les différentes grandeurs que la considère ne dépendent plus de vraiment de l'impulsion $\pv$ (ou $\qv$) mais seulement de $\pv^2$ et donc de sa norme $p = \| \pv \|_{2}$ (ou q). \\


Ainsi, pour diminuer les erreurs commises par l'approximation et utiliser les informations exactes que l'on connait sur l'évolution de $V_k$, l'équation de flot peut se réécrire sous la forme du système
\begin{align}
	...
	\partial_t W_k(\rho) & = 
	\begin{aligned}[t]
		& \partial_\rho I_1(\rho)
	\end{aligned}
\end{align}

Avec les notations
\begin{equation}
	m_k^2(\rho) = \partial_\phi^2V_k |_\rho
\end{equation}
\begin{equation}
...
Il reste alors une dernière étape avant de parvenir aux équations finales. En effet, nous n'avons pas ici réalisé l'adimensionnement fondamental du RG. Comme décrit en \refsec{}, pour cela il suffit d'introduire de nouvelles grandeurs notées d'un $\tilde{.}$ de la même manière. On considère aussi que $k$ s'exprime en unité de $\Lambda$ et donc qu'il varie entre $1$ et $0$, de cette manière $k$ et $s$ sont confondus. Concrètement cela revient à introduire, en utilisant le $Z_k$ défini en \refsec{}, 
\begin{equation}
\begin{split}
...
Ce changement de variable en plus d'être nécessaire pour trouver le point fixe apporte de grand changement dans la forme des équations. En effet, on multiplie les expressions par des facteurs contenant $k$ ou $Z_k$ qui possèdent des dérivées non nulles lorsque l'on leur applique l'opérateur différentiel $\partial_t$. En outre $\partial_\rho \Delta_k(p,\rho)$ représentait la dérivée par rapport à $\rho$ de $\Delta_k$ à $p$ fixé. Or comme maintenant les fonctions dépendent de $\tp$ et non plus de $p$ il faut calculer cette même grandeur à $\tp$ fixé. Ceci explique alors comme on en arrive à la nouvelle forme du problème que nous avons essayé de résoudre numériquement :\\
